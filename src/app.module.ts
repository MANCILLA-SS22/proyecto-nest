...
@Module({
  ...
  }), TasksModule, StudentsModule, HelloModule],
...
//Básicamente es una herencia, pero esta vez no es aplicada sólo a clases habituales, sino que también se aplican a funciones. Entendiendo este concepto, podemos decir que con el 
//decorador @Module, estamos indicando que lo que se encuentra entre paréntesis será tratado como un módulo.
...
//Se utiliza un ConfigModule.forRoot(), esto permite inicializar el contexto del módulo de configuración para que sea conocido y contemplado por el aplicativo principal y en los módulos internos. 
//Luego, tenemos el cambio en MongooseModule, donde esta vez estamos haciendo una inicialización dinámica. Nota cómo el configModule también se pasa al import de MongooseModule, así también como 
//el ConfigService se inyecta para que pueda ser utilizado dentro de MongooseModule.
...
//Config extiende de ConfigService, es por ello que aplicando un get podemos apreciar como se accede a una variable de entorno desde Nest. config.get<string>(‘Nombre de la variable’) buscará el 
//archivo .env y tomará la variable con ese nombre, en caso de no encontrarla, devolverá undefined. Solo queda colocar en el .env la string de conexión de mongo.
